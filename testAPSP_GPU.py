...
def work(n, m, method):
	...
	# GPU 
	t1 = time()
	r = calc(graph = g, useCUDA = True, srclist = None)
	t2 = time()
...
if __name__ == '__main__':
	...
	# 干掉热热启动
	t, dist = work(100, 200, 'dij')

	# CSV name 每次运行都是一个新的文件名
	filename = f"./testResult/apsp/test_GPU_{str(time())[11:]}.csv"
		...
		for d in ds:
			for method in methods:
				try:
					...
					TimeGPUs.append(t)
					np.savez(f'./testResult/apsp/ans_GPU/dist_{n}_{n*d}_{method}', dist)	
				except:
					Ns.append(n)
					Ms.append(n*d)
					Methods.append(method)
					TimeGPUs.append("can not calc")
					np.savez(f'./testResult/apsp/ans_GPU/dist_{n}_{n*d}_{method}', None)	
			
			# save
			df = pd.DataFrame({'n':Ns, 'm':Ms, 'method':Methods, 'timeGPU':TimeGPUs})
			df.to_csv(filename)
